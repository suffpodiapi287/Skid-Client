...
class LegitAura : Module(name = "LegitAura", category = ModuleCategory.COMBAT) {
    ...
    override fun onEnable() {
        ...
        if (rangeModeValue.equals("Universocraft") && autoconfig.get()) {
            ...
            fovValue.set(110)
            FDPClient.hud.addNotification(Notification("KillAura", "New settings applied", NotifyType.WARNING, 1000, 500))
    ...
    fun onStrafe(event: StrafeEvent) {
        val targetStrafe = FDPClient.moduleManager.getModule(TargetStrafe::class.java)!!
        ...
        if (currentTarget != null && RotationUtils.targetRotation != null) {
            when (rotationStrafeValue.get().lowercase()) {
                ...
                "silent" -> {
                    ...
                    (RotationUtils.targetRotation as? Rotation)?.applyStrafeToPlayer(event)
                    event.cancelEvent()
                }
            }
        }
    ...
    public fun isEnemy(entity: Entity?): Boolean {
        if (entity is EntityLivingBase && (FDPClient.moduleManager[Target::class.java]!!.deadValue.get() || isAlive(entity)) && entity != mc.thePlayer) {
            if (!FDPClient.moduleManager[Target::class.java]!!.invisibleValue.get() && entity.isInvisible())
                return false

            if (FDPClient.moduleManager[Target::class.java]!!.playerValue.get() && entity is EntityPlayer) {
                ...
                val teams = FDPClient.moduleManager[Teams::class.java] as Teams

                return !teams.state || !teams.isInYourTeam(entity)
            }

            return FDPClient.moduleManager[Target::class.java]!!.mobValue.get() && EntityUtils.isMob(entity) || FDPClient.moduleManager[Target::class.java]!!.animalValue.get() &&
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        // Call attack event
        FDPClient.eventManager.callEvent(AttackEvent(entity))
        ...
        val criticals = FDPClient.moduleManager[Criticals::class.java] as Criticals
